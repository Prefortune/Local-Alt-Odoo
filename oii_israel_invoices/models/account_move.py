...
class AccountMove(models.Model):
    ...
    def action_post(self):
        ...
        if company.enable_taxes_integration:
            _logger.info("Tax Integration is Enable")
            needs_confirmation_number = self.filtered(
                lambda x: x.move_type in ['out_invoice', 'out_refund', 'il_invoice_receipt']
                          ...
                          and x.amount_untaxed >= self.env.company.rt_taxes_minimum_untaxed_amount
                          and x.amount_tax
            )
            for move in needs_confirmation_number:
                action = move.get_conf_number()
                _logger.info("Initialise Israel Invoice API")
    ...
    def prepare_invoice_details(self):
        ...
        values = {
            ...
            "discount": 0.00,
            "payment_amount": round(self.amount_untaxed, 2),
            "vat_amount": round(self.amount_tax, 2),
            "payment_amount_including_vat": round(self.amount_total, 2),
            # "invoice_note": "הערות",
            # "Action": 0,
