...
class SEOURL(models.AbstractModel):
    ...
    def _check_seo_url(self, vals, record_id=0):
        ...
        if value:
            original_value = self.env['ir.http']._slugify_one(value, max_length=0, field_name='seo_url')
            counter = 0
            while True:
    ...
    def write(self, vals):
        for r in self:
            checked_vals = r._check_seo_url(vals, record_id=r.id)
            super(SEOURL, r).write(checked_vals)
        return True
    ...
    def _check_url_field_lang(self, vals):
        ...
        """
        field_lang = vals.get(self._seo_url_field_lang) or self.seo_url_language
