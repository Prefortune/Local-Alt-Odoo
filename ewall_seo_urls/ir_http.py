    ...
    def to_python(self, value) -> models.BaseModel:
        ...
        if field and field in request.registry[self.model]._fields:
            ...
            for lang in langs:
                ...
                # תמיכה בהתנהגות של multi-website: מוצר ללא website_ids פתוח לכולם
                model_obj = env[self.model]
                if 'website_ids' in model_obj._fields and hasattr(request, 'website') and request.website:
                    domain += ['|', ('website_ids', '=', False),
...
class IrHttp(models.AbstractModel):
    ...
    def _get_traget_record(cls, value: str) -> str:
        ...
        for model in ALLOWED_MODULES:
            target_record = request.env[model].search(
