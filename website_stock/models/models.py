...
class ProductProduct(models.Model):
    ...
    wk_in_stock_msg = fields.Char(string='In Stock Message', translate=True, default="In Stock")
    wk_out_of_stock_msg = fields.Char(string='Out of Stock Message', translate=True, default='This product has gone out of Stock!')
...
class ProductTemplate(models.Model):
    ...
    def wk_get_product_vatiants(self):
        ...
        result = {
            ...
            'help': action.help,
            'type': action.type,
            'views': [[list_view_id.id, 'list'], [form_view_id.id, 'form']],
...
class Website(models.Model):
    ...
    def get_config_settings_values(self):
        ...
        stock_config_values = self.env['website.stock.config.settings'].sudo().search(
            [('website_id', '=', website.id),('is_active', '=', True)], limit=1)
        
        if stock_config_values:
            res = {
                'wk_display_qty': stock_config_values.wk_display_qty,
                'website_id': stock_config_values.website_id,
    ...
    def get_packaging_enabled_ws(self, current_website):
        return getattr(current_website, "packaging_enabled", False)
    ...
    def get_product_stock_qty(self, product_obj, type_stock):
        ...
        # CODE For Website Product Pack Compatibility
        if product_obj.type in ['service', 'consu'] and not product_obj.is_storable:
            return 10
        qty_hand, virtual, outgoing = product_obj.sudo().qty_available, product_obj.sudo().virtual_available, product_obj.sudo().outgoing_qty
    ...
    @api.model
    def cart_line_stock_validate(self, product_id=None, added_qty=0.0):
