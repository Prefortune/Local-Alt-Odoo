...
class WebsiteStockConfigSettings(models.Model):
    ...
    in_stock_text = fields.Char(string="Text color(In Stock)", size=7, default="#FFFFFF")
    ...
    out_stock_text = fields.Char(string="Text color(Out Stock)", size=7, default="#FFFFFF")
    ...
    def _check_unique_config_website(self):
        active_ids = self.search([('website_id', '=', self.website_id.id)])
        if len(active_ids) > 1:
            raise ValidationError(_(f'A single configuration can be configured for {self.website_id.name}!'))
    ...
    def set_product_default_value(self):
        ...
        record = self.search([('website_id', '=', website.id),('is_active', '=', True)], limit=1)
