...
class Website(models.Model):
    ...
    def _tracking_send_by_api_is_denied(self, service, product_data: dict, order, event_type: str) -> bool:
    ...
    def _tracking_event_data(
        ...
        for service in website.tracking_service_ids.filtered('is_internal_logged'):
            if website._tracking_do_logging(service):
                ...
                api_send_is_denied = website._tracking_send_by_api_is_denied(
                    service=service, product_data=product_data, order=order, event_type=event_type,
                )
                log_data.update({'api_send_is_denied': api_send_is_denied})
                if api_send_is_denied or not service_data.get('run_script'):
