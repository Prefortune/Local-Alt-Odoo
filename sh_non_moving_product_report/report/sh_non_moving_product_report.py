...
class NonMovingProduct(models.AbstractModel):
    _name = 'report.sh_non_moving_product_report.sh_non_moving_product'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if sh_selection=='warehouse':
            self._cr.execute('''select id,date_done from stock_picking where
            date_done::date >=%s and date_done::date <=%s and location_id IN %s
        ...
        # ========== Print selected location wise report =================
            ...
            date_done::date>=%s and date_done::date<=%s and location_id IN %s and state='done'and picking_type_id IN
            (select id from stock_picking_type where code='outgoing')''',
            ...
            if picking:
                for product in products:
                    ...
                    if stock_move:
                        ...
                        (select id from stock_picking_type where code='outgoing'))  and date::date <= %s and state='done' ORDER BY id DESC LIMIT 1 ''',[product.id,str(sh_to_date)])
                        stock_move = self._cr.dictfetchall()
                        if sh_selection=='warehouse':
