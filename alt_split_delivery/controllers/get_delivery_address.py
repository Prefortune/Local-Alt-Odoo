...
class SplitDeliveryController(http.Controller):
    ...
    @http.route(['/my/orders/<int:order_id>/<string:token>/split_delivery'], type='http', auth="public", website=True)
    def show_split_delivery_page(self, order_id, token ,**kwargs):

        order = request.env['sale.order'].sudo().browse(order_id)
        if not order:
            raise AccessDenied()
        
        if not order.split_access_token or order.split_access_token != token:
            _logger.warning("Invalid split access token for order %s", order_id)
            raise AccessDenied()
    ...
    @http.route(['/my/orders/<int:order_id>/<string:token>/split_delivery/save'], type='http', auth="public", website=True, methods=['POST'])
    def save_split_delivery(self, order_id, token, **post):
        ...
        ordered_qty_map = {
            ...
            for line in order.order_line.filtered(lambda l: l.product_id.type != 'service' and l.price_unit > 0)
        }
        ...
        if error_messages:
            return request.render("alt_split_delivery.split_delivery_error_page", {
                ...
                'order': order_id,
                'split_access_token' : order.split_access_token,
                'website': website_id,
            })
        else:
            # return False
            ...
            for i in range(count):
                ...
                if partner_id:
                    ...
                    partner.write({
                        ...
                        'phone' : post.get(f'phone_{i}',''),
                        'name' : post.get(f'name_{i}',''),
                    })
                else:
            ...
            order.message_post(body=f'Split delivery address confirmed by customer')
            website_url = request.env['website'].get_current_website().domain
            if website_url:
                return request.redirect(f'{website_url}')
            else:
                return request.redirect(f'/my/orders/{order.id}/{order.split_access_token}/split_delivery')
