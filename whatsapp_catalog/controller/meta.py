...
class WebHook2Ext(WebHook2):
    ...
    def meta_webhook(self, **kw):
        ...
        self._get_catalog_payment_status(data, provider)
                ...
                for mes in (
                    ...
                    for partner in partners:
                        if mes.get("type") == "order" and partner:
                            ...
                            )._send_catalog_commerce_manager()
        return super().meta_webhook(**kw)

    def _get_catalog_payment_status(self, data, provider):
        if data and provider:
            try:
                payment = {}
                try:
                    payment = data.get("entry")[0].get('changes')[0].get('value').get('statuses')[0].get('payment')
                except:
                    payment = payment.get("entry")[0].get("changes")[0].get("value").get('interactive')[0].get('payment')
