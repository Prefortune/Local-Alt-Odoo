...
class AccountPayment(models.Model):
    ...
    unique_vendor_count = fields.Integer(
        string='Unique Vendor Count',
        compute='_compute_unique_vendor_count',
        store=True
    )

    @api.depends('partner_id')
    def _compute_unique_vendor_count(self):
        for rec in self:
            if rec.partner_type != 'supplier' or not rec.partner_id:
                rec.unique_vendor_count = 0
                continue

            # Find first payment by this vendor
            payments = self.env['account.payment'].search([
                ('partner_id', '=', rec.partner_id.id),
                ('partner_type', '=', 'supplier')
            ], order='date asc, id asc', limit=1)

            # Set count to 1 only for the first payment
            rec.unique_vendor_count = 1 if payments and payments.id == rec.id else 0
    ...
    def get_withholding_condition(self):
        ...
        for rec in self:
            ...
            elif rec._fields.get('receipt_id',
                                  False) and rec.receipt_id and rec.withholding_payment:
                withholding = True
