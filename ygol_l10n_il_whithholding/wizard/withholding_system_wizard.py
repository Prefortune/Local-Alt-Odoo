...
class Israel1000System(models.TransientModel):
            ...
            [
                ...
                ('invoice_ids', 'in', self.env['account.move'].search(
                    [
                        ...
                        # ('state', '=', 'draft')  # TMP: commented out for testing
                    ]
                ).ids),
            ]
        )
        # Ticket HT01460 updated domain in 'res.partner'
        main_data_list = []
        global main_counter
    ...
    def upload_1000_txt_report(self):
        ...
        first_text_file = '/tmp/first_text_file.txt'

        if str(self.extraction_filename.split(".")[1]) != 'txt':
            raise ValidationError("Please upload data in .txt file extension only..!!")
