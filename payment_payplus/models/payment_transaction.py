...
class PaymentTransaction(models.Model):
    ...
    def _get_specific_processing_values(self, processing_values):
        ...
        Products = []
        sale_order = self.env['sale.order'].sudo().search([('name','=',processing_values['reference'].split('-'))])
        if sale_order:
            customer = {
                "customer_name" : sale_order.partner_id.name,
                "email" : sale_order.partner_id.email,
                #"vat_number" : sale_order.partner_id.vat,
                "phone" : sale_order.partner_id.phone,
                "address" : sale_order.partner_id.street,
                "postal_code" : sale_order.partner_id.zip,
                "city" : sale_order.partner_id.city,
                "country_iso" : sale_order.partner_id.country_id.code,
            }
            if sale_order.order_line:
                for line in sale_order.order_line.filtered(lambda l : l.price_total > 0):
                    if line.tax_id:
