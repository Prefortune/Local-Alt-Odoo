...
class HideVariants(http.Controller):
    ...
    def fetch_product_website_hide_variants(self, tmpl_id):
        res = request.env['product.product'].sudo().browse(tmpl_id).is_website_hide_variants
        _logger.info("fetch_product_website_hide_variants ----> %s  , %s", request.env['product.product'].sudo().browse(tmpl_id) , res)
        return res
