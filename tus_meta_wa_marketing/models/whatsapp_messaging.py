...
class WhatsAppMessaging(models.Model):
    ...
    def _get_current_model_template(self):
        domain = [("model", "in", ["res.partner", "whatsapp.messaging.lists"]),
                  ("state", "=", "added")]
        provider_id = self.user_id.provider_ids.filtered(lambda x: x.company_id == self.company_id)
    ...
    def action_view_documents_filtered(self):
        action = self.env["ir.actions.actions"]._for_xml_id(
            "tus_meta_whatsapp_base.whatsapp_history_action"
        )
    ...
    def action_schedule_date(self):
        self.ensure_one()
        action = self.env.ref(
            "tus_meta_wa_marketing.whatsapp_messaging_schedule_date_action"
        ).read()[0]
        action["context"] = dict(
    ...
    def action_test_whatsapp_marketing(self):
        for record in self:
            return {
                ...
                'target': 'new',
                'view_id': self.env.ref('tus_meta_wa_marketing.test_whatsapp_marketing_wizard_form').id,
