    ...
    def confirm_profit_approve(self):
        ...
        if sale_order.parent_id.name != self.env.user.name:
            if sale_order.manager_approval != True and sale_order.email_sent_manager != True:
                ...
                mail.send()
                manager_user = sale_order.parent_id
                user = self.env['res.users'].search([('name', '=', manager_user.name)], limit=1)

                if not manager_user:
                    raise ValidationError(_(
                        "The manager (%s) does not have a linked internal user (user_id). "
                        "Please assign a user to the manager via the Contacts or Employees menu."
                    ) % sale_order.parent_id.name)
