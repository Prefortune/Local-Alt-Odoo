...
class ShopifyCustomer(models.Model):
    ...
    def _prepare_customer_dict(self, customer, default_address, shopify_connector):
        if default_address:
            ...
            default_address_country_name = default_address.get('country')
    ...
    def _get_country_and_state_ids(self, country_name, province_name):
        ...
        if province_name and country_id:
            state = self.env['res.country.state'].search([('country_id', '=', country_id),('name', '=', province_name)])
            if state:
                state_id = state.id
    ...
    def _prepare_extra_address_dict(self, extra_address, parent_id, customer_id, shopify_connector):
        ...
        extra_address_country_name = extra_address.get('country')
        extra_address_phone = extra_address.get('phone')
